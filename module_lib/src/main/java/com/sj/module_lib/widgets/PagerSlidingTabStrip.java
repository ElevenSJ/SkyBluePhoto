...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    ...
    private int tabIconPadding = 10;
    private int dividerWidth = 1;

    private int tabTextSize = 16;//字体大小
    private int tabTextColor = 0x66000000;//字体颜色
    private int selectedTabTextColor = 0xFF660000;//选中后的字体颜色
    ...
    public void notifyDataSetChanged() {
        ...
        for (int i = 0; i < tabCount; i++) {
            ...
            if (pager.getAdapter() instanceof IconTabProvider) {
                addIconTab(i, pager.getAdapter().getPageTitle(i),((IconTabProvider) pager.getAdapter()).getPageIconResId(i),((IconTabProvider) pager.getAdapter()).getIconType());
            } else {
    ...
    private void addIconTab(final int position, CharSequence pageTitle, int pageIconResId, int resType) {
//        ImageButton tab = new ImageButton(getContext());
//        tab.setImageResource(resId);
//        addTab(position, tab);
        TextView tab = new TextView(getContext());
        tab.setText(pageTitle==null?"":pageTitle.toString());
        Drawable drawable = getResources().getDrawable(pageIconResId);
        if (drawable!=null) {
            if (resType == Gravity.LEFT) {
                tab.setCompoundDrawablesWithIntrinsicBounds(drawable,
                        null, null, null);
            } else if (resType == Gravity.RIGHT) {
                tab.setCompoundDrawablesWithIntrinsicBounds(null,
                        null, drawable, null);
            } else if (resType == Gravity.BOTTOM) {
                tab.setCompoundDrawablesWithIntrinsicBounds(null,
                        null, null, drawable);
            } else {
                tab.setCompoundDrawablesWithIntrinsicBounds(null,
