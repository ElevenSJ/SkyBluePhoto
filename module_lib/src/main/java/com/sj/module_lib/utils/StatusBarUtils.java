...
public class StatusBarUtils {
    ...
    public void init() {
        ...
        if (mColor != -1) {
            //设置了状态栏颜色
            addStatusViewWithColor( mColor);
        }
        if (mDrawable != null) {
            //设置了状态栏 drawble，例如渐变色
            addStatusViewWithDrawble(mDrawable);
        }
        if (isDrawerLayout()) {
        ...
        if (isActionBar()) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                ViewGroup rootView = (ViewGroup) this.activity.getWindow().getDecorView().findViewById(android.R.id.content);
                rootView.setPadding(0, getStatusBarHeight(this.activity) + getActionBarHeight(this.activity), 0, 0);
            }
        }
    }
     ...
     */
    public void fitsSystemWindows(boolean isTrue) {
        ViewGroup contentFrameLayout = (ViewGroup) activity.findViewById(android.R.id.content);
        View parentView = contentFrameLayout.getChildAt(0);
        if (parentView != null && Build.VERSION.SDK_INT >= 14) {
            parentView.setFitsSystemWindows(isTrue);
            //布局预留状态栏高度的 padding
            if (isTrue&&parentView instanceof DrawerLayout) {
     ...
     *
     */
    public StatusBarUtils addStatusViewWithColor(int color) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (isDrawerLayout()) {
     ...
     * 添加状态栏占位视图
     */
    private void addStatusViewWithDrawble( Drawable drawable) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            //占位状态栏
            View statusBarView = new View(activity);
            ViewGroup.LayoutParams lp = new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
     ...
     *
     */
    public StatusBarUtils fullScreen() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
